...
const profilePopup = new PopupWithForm('#edit_form', () => {
  userInfo.setUserInfo({userName:popupName.value, userInfo:popupTitle.value})
  profilePopup.close();
}, () => {
...
const newCardPopup = new PopupWithForm('#location_form',() => {
  const card = {};
  card.link = cardPopupLink.value;
  card.name = cardPopupName.value;
  const cardEntity = new Card(card, ".element", viewCardPopup.open.bind(viewCardPopup));
...
const cardsList = new Section({
  items: initialCards,
  renderer: (item) => {
    const cardObject = new Card(item, ".element", viewCardPopup.open.bind(viewCardPopup));
